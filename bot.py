...
async def cmd_start(update: Update, _context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    bienvenida = (
        ...
        f"   6 días de programación completa con enfoque, extras y niveles.\n\n"
        f"🏆 */benchmark* — WOD clásico directo\n"
        f"   Fran, Cindy, Helen, Murph o la versión BRUTUS de alguno.\n\n"
        f"💡 */tip [movimiento]* — Tip técnico inmediato\n"
        f"   Ejemplo: `/tip rope climb` · `/tip double under` · `/tip snatch`\n\n"
...
async def cmd_benchmark_from_query(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    prompt = """
Genera un WOD Benchmark clásico de CrossFit (ej. Fran, Cindy, Helen, Murph, etc.) o una versión brutal tuya de uno de ellos, utilizando solo el equipo disponible en el box.
Aplica el formato WOD DEL DÍA. El nombre debe ser el del Benchmark original o una variación estilo BRUTUS.
"""
    try:
...
async def handle_wod_tipo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        f"Tipo: {tipo}\n\n¿Cuántos minutos de tu vida le dedicas al dolor?\nElige o escribe un número:",
...
async def handle_wod_tiempo_custom(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if texto.isdigit() and 5 <= int(texto) <= 60:
        ...
        await update.message.reply_text(
            f"Tiempo: {texto} minutos. Bien.\n\n¿Qué tan arrepentido quieres salir?",
            reply_markup=teclado_intensidad()
        )
        return WOD_INTENSIDAD
    else:
        await update.message.reply_text("Oye, escribe un número entre 5 y 60, no te hagas.")
        return WOD_TIEMPO_CUSTOM
...
async def handle_wod_enfoque(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    enfoque = enf_map[query.data]
    context.user_data['wod_enfoque'] = enfoque
    
    await query.edit_message_text(
        f"Enfoque: {enfoque}\n\nPerfecto. ¿Para quién genero las escalas?",
...
async def handle_wod_niveles(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    niveles = niv_map[query.data]
    context.user_data['wod_niveles'] = niveles
...
async def generate_wod(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ud = context.user_data
    historial = ', '.join(ud.get('wods_generados', [])[-5:]) if ud.get('wods_generados') else 'Ninguno'
    estilo = estilo_aleatorio()

    tipo_raw = ud['wod_tipo']
    if tipo_raw == 'RANDOM':
        tipo_instruccion = "RANDOM — Tú eliges el tipo (AMRAP, FOR TIME, EMOM, CHIPPER, LADDER o TABATA). Especifícalo en el output."
    else:
        tipo_definiciones = {
            'AMRAP': 'AMRAP — As Many Rounds/Reps As Possible en el tiempo dado. Estructura: lista de movimientos con reps por ronda.',
            'FOR TIME': 'FOR TIME — Completar X rondas o X reps lo más rápido posible. Estructura: N rounds for time o total de reps for time.',
            'EMOM': 'EMOM — Every Minute On the Minute. Cada minuto se ejecuta un movimiento o secuencia. Estructura: min 1 / min 2 / etc.',
            'CHIPPER': 'CHIPPER — Lista larga de ejercicios para completar en orden, una vez cada uno. Estructura: una sola ronda, muchos movimientos.',
            'LADDER': 'LADDER — Reps suben o bajan cada ronda (ej. 2-4-6-8...). Estructura: progresión de rondas con incremento.',
            'TABATA': 'TABATA — 20 segundos trabajo / 10 segundos descanso, 8 intervalos por movimiento. Estructura: Tabata de X movimientos.'
        }
        tipo_instruccion = tipo_definiciones.get(tipo_raw, tipo_raw)

    prompt = f"""
Genera un WOD de CrossFit con estos parámetros. El TIPO es OBLIGATORIO y no lo puedes cambiar bajo ninguna circunstancia:

▶ TIPO (NO CAMBIAR): {tipo_instruccion}
▶ Duración total: {ud['wod_tiempo']} minutos
▶ Intensidad: {ud['wod_intensidad']}
▶ Enfoque muscular: {ud['wod_enfoque']}
▶ Niveles a mostrar: {ud['wod_niveles']}

EQUIPO DISPONIBLE (solo usa esto):
Barras olímpicas y discos, mancuernas, rack para pull-ups,
cuerda para rope climb, cuerda sola, sand bags, cajones,
kettlebells, cuerdas de salto (jump rope), air bike.

HISTORIAL RECIENTE (no repitas movimientos principales):
{historial}

Sigue exactamente el formato de output definido en tu sistema para un WOD DEL DÍA.
El nombre del WOD debe ser estilo [{estilo}] de tu banco de estilos.
"""
...
async def handle_sem_enfoque(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    enfoque = enf_map[query.data]
    context.user_data['sem_enfoque'] = enfoque
...
async def handle_sem_duracion_btn(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        f"Duración: {mins} min\n\n¿Qué incluyo en cada día? (puedes seleccionar varios)",
...
async def handle_sem_duracion_custom(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if texto.isdigit():
        ...
        await update.message.reply_text("Oye, escribe un número.")
        return SEM_DURACION_CUSTOM
...
async def generate_semana(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    prompt = f"""
Genera una SEMANA COMPLETA de programación de CrossFit (Lunes a Domingo) con estos parámetros:
- Enfoque de la semana: {ud['sem_enfoque']}
- Duración por sesión: {ud['sem_duracion']} minutos
- Elementos a incluir por día: {', '.join(extras)}
- Niveles a mostrar: {ud['sem_niveles']}
...
Sigue exactamente el formato de output definido en tu sistema para WOD SEMANAL.
Cada día debe tener un WOD nombrado con un estilo aleatorio de tu banco de estilos, sin repetir movimientos de forma tonta.
"""
    ...
    try:
        ...
        # Split on newlines to avoid breaking Markdown elements mid-block
...
async def cmd_benchmark(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    prompt = """
Genera un WOD Benchmark clásico de CrossFit (ej. Fran, Cindy, Helen, Murph, etc.) o una versión tuya brutal de uno de ellos, utilizando solo el equipo que tenemos disponible.
Aplica el formato WOD DEL DÍA. El nombre debe ser el del Benchmark original o una variación tuya estilo BRUTUS.
"""
    try:
        response = await client.chat.completions.create(
            model="gpt-4o",
...
async def cmd_tip(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.args:
        await update.message.reply_text("Dime sobre qué movimiento quieres el tip. Ejemplo: /tip rope climb")
        return
    ...
    prompt = f"""
Dame un tip de coaching avanzado, brutal y directo para mejorar en: {movimiento}.
Nada de motivación barata, solo biomecánica, eficiencia y actitud BRUTUS.
"""
    try:
        response = await client.chat.completions.create(
            model="gpt-4o",
...
async def cmd_nombre(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    prompt = "Genera 5 nombres de WODs utilizando diferentes estilos de tu banco de estilos. Formatea como una lista con viñetas indicando el estilo usado."
    try:
        response = await client.chat.completions.create(
            model="gpt-4o",
            messages=[
...
def main() -> None:
    ...
    print("🌪️ BRUTUS Bot v2.0 — En línea. Esperando víctimas...")
